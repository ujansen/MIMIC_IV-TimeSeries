...
class TimeSeriesModel(nn.Module):
    def __init__(self, args: argparse.Namespace):
        ...
        notes_emb_size = args.hid_dim*3 # Use *3 during pretraining
        ...
        if self.pretrain:
            ...
            self.forecast_head = nn.Linear(notes_emb_size, args.V) # Change to ts_emb_size when NOT using notes
            # self.notes_head = nn.Sequential(nn.Linear(notes_emb_size, args.hid_dim),
            #                             nn.Tanh(),
            #                             nn.Linear(args.hid_dim, args.V))
            self.binary_head = nn.Linear(args.V, 1) # Keep 2 for multilabel 1 for binary (Make it 3 when using checkpoints from output_notes)
