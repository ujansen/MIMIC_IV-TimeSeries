...
class PretrainEvaluator:
    ...
    def evaluate(self, model, dataset, split, train_step):
        ...
        # self.args.logger.write('Indices are', dataset.splits[split])

        if split not in self.io:
            batches = []
            eval_ind = dataset.splits[split]
            # self.args.logger.write(f"Eval indices for split {split}: {eval_ind}")
            num_samples = len(eval_ind)
            for start in tqdm(range(0,num_samples,self.args.eval_batch_size),
                              desc='Generating io for eval split '+split, file=sys.stdout):
        ...
        model.eval()
        pbar = tqdm(self.io[split], desc='running forward pass', file=sys.stdout)
        loss, count = 0,0
        for batch in pbar:
